...
## D-019 — Замена запроса в одной вкладке атомарна и упорядочена по sequence
...
блокировкой. Каждая запись несёт монотонный `sequence`, назначаемый синхронно
в `prepare()` в порядке действий пользователя. Внутри блокировки более старый
запрос, уже проигравший гонку, отказывается сохраняться и не удаляет
победителя. Порядок определяется `sequence`, а не порядком разрешения промисов.
...
`test:coverage` измеряет 17 перечисленных critical modules, а не весь runtime.
