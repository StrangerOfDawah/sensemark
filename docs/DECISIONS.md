...
Session entry удаляется при consume, а request-specific in-memory guard не даёт
двум одновременным consumer начать один перевод. После перезапуска worker
durable remove остаётся источником истины. Перед direct fallback общая language
policy пропускает уверенный русский текст без provider/cache/UI.
...
`test:coverage` измеряет 14 перечисленных critical modules, а не весь runtime.
