...
Нажмите на иконку Sensemark в панели Chrome — откроется компактное окно ручного перевода. В него можно вставить до 5000 символов и запустить перевод кнопкой или сочетанием <kbd>⌘</kbd>/<kbd>Ctrl</kbd> + <kbd>Enter</kbd>.

Ручной режим использует тот же потоковый перевод, локальный фильтр русского текста, многоязычные секции и янтарное объяснение неизвестных имён и названий. Результат можно скопировать; изменение текста или кнопка остановки сразу отменяют незавершённый API-запрос.
...
Для каждого pull request и каждого изменения `main` GitHub Actions запускает unit- и DOM-интеграционные тесты, проверяет обязательные пороги покрытия (90% строк, 75% ветвей, 95% функций), структуру Manifest V3 и полный состав архива. Любая ошибка останавливает job и тестовая сборка не публикуется. После успешной проверки workflow прикрепляет распаковываемый артефакт `sensemark-pr-N`.
...
3. Вставьте ключ в настройках и нажмите **Проверить ключ**
...
| Вручную | Нажать на иконку расширения → вставить текст → «Перевести» |
...
| `manual-translation.js` | Подготовка и разбор ручного перевода без дублирования API-логики |
| `popup.html` · `popup.css` · `popup.js` | Компактное окно ручного потокового перевода |
| `ui-scale.js` | Ограничение скорости и шага масштабирования карточки |
| `content.js` | Карточка на странице, разбор контекста, масштаб и размер |
| `options.html` · `options.js` | Страница настроек |
| `icons/` | Иконки 16–128 |

Ключ хранится в `chrome.storage.local`. Выделенный или вручную введённый текст и, для короткого выделения, окружающее предложение отправляются напрямую в OpenAI для перевода. У расширения нет сервера разработчика и аналитики; подробности — в [политике конфиденциальности](PRIVACY.md).
